...
while response.lower() == 'y':
    ...
    # Ask user if they want to review previous entries and open the text file
    if sfile.lower() == 'y':
        read = input('\nWould you like to review the previous logged entries? (Y or N) ')
        read = check(read)
        if read.lower() == 'y':
    ...
    print(choices)
    x = input('Enter the number associated with the operation you would like to run: ')
    exit(x)
